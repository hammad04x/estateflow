...
const UpdateProperty = () => {
  ...
  const [imgPreview, setImgPreview] = useState(
    initial.image ? `/uploads/${initial.image}` : initial.img ? `/uploads/${initial.img}` : null
  );
  ...
  const handleUpdate = async () => {
    ...
    Object.entries(form).forEach(([key, value]) => {
      // append all keys except image when null
      if (value !== null && value !== undefined) fd.append(key, value);
    });

    try {
      // IMPORTANT: backend route expected at /updateproperty/:id (no /admin prefix)
    ...
    return (
      <main className="admin-panel-header-div">
        <div className="empty-state-center">
          ...
          <button className="primary-btn" onClick={() => navigate("/admin/properties")}>
            Go Back
          </button>
    ...
    >
      <div className="form-header-top" style={{ marginTop: 4 }}>
        <div className="header-top-left">
          <button className="header-back-btn" onClick={() => navigate("/admin/properties")}>
